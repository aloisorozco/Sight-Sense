...
class Obstacle:
    ...
    def set_position_and_size(self):
        self.position = [(self.xyxy[0] + self.xyxy[2]) / 2,
    ...
    def __str__(self):
        # # +", size: " + str(self.size) + " pos: " + str(self.position) + " in ROI: " + str(not self.is_not_in_ROI)
        # return self.name + (" detected" if self.is_not_in_ROI else " in front")
        return f"{self.name} ({self.pos})"  # use position
